...
- sticky：元素会先根据正常文档流进行定位，相对于最近的拥有“滚动机制”的祖先和最近块级祖先的值进行偏移，到达相对位置后，元素会固定在该位置；例如某些网站头部下拉一定位置后固定的导航栏
...
- display 为 inline-block、flow-root、table-cell、table-caption 的元素
- display 值为 flex、inline-flex、grid、inline-grid 元素的直接子元素，但是它们本身不能是 flex、grid 或是 table 容器
- ...
